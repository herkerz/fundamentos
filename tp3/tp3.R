...
# df <- acath %>% filter(!is.na(choleste))
...
idx_sample = get_sample(data = df, percentage_tr_rows=0.8, seed = 111)
...
modelo_logistic_simple = glm(sigdz ~ choleste, family=binomial,data = df_train)
...
predicted_response <- round( predict(modelo_logistic_simple,df_test,type = "response"))

outcomes <- table(predicted_response, actual_response )

confusion <- conf_mat(outcomes)
autoplot(confusion,type="heatmap") + labs(title="Confusion Matrix Modelo Simple",caption = "Fig 4.")
summary(confusion, event_level = "second")

grid.arrange(tableGrob(summary(confusion,event_level = "second") %>% 
                         slice(1,11,12,13) %>% 
...
modelo_logistic_no_cat <- glm(sigdz ~ choleste + age + cad.dur, family=binomial,data = df_train)

actual_response <- df_test$sigdz
predicted_response <- round( predict(modelo_logistic_no_cat,df_test,type = "response"))

outcomes <- table(predicted_response, actual_response )
confusion <- conf_mat(outcomes)

autoplot(confusion,type="heatmap") + labs(title="Confusion Matrix Modelo No categoricas",caption = "Fig 5.")
...
modelo_logistic_cat <- glm(sigdz ~ sex + choleste + age + cad.dur, family=binomial,data = df_train)
summary(modelo_logistic_cat)
actual_response <- df_test$sigdz 
predicted_response <- round( predict(modelo_logistic_cat,df_test,type = "response"))

outcomes <- table(predicted_response, actual_response )
confusion <- conf_mat(outcomes)

autoplot(confusion,type="heatmap") + labs(title="Confusion Matrix Modelo Categoricas",caption = "Fig 6.")
...
datos <- data.frame(localidad,frecuencia_asistencia)
ggplot(datos,aes(localidad,frecuencia_asistencia)) +
  geom_boxplot() +
  labs(title="Distribucion de las variables",
...
tapply( frecuencia_asistencia,localidad, summary)
...
desviaciones <- tapply(frecuencia_asistencia, localidad, sd, na.rm = TRUE)

desviaciones

max(desviaciones)/min(desviaciones) #tendría que dar 1
...
data(nottem)
nottem_ts = ts(nottem, start=c(1920,1),freq=12)
start(nottem_ts)
end(nottem_ts)
...
AP<- ts(nottem_ts,frequency = 12, start = c(1920,1), end = c(1938,12))

modelo_arima <- auto.arima(AP, stationary = FALSE, seasonal = TRUE)
summary(modelo_arima)
forecast_temp<- forecast(modelo_arima, h =12)
